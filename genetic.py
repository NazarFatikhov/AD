...
def create_distribution(results):
    # results = [
    # 	k_n - k_1 * pk_1_1 + k2 * pk_1_2 + … + k_n-1 * pk_1_n-1,
    # 	k_n - k_1 * pk_2_1 + k2 * pk_2_2 + … + k_n-1 * pk_2_n-1,
    # 	….
    # 	k_n - k_1 * pk_count_1 + k2 * pk_count_2 + … + k_n-1 * pk_count_n-1
    # ]
    # Берем величины 1/results[i]
    # Reverses = [1/results[1], 1/results[2], …, 1/results[count]]
    # sum_r = sum(reverses)
    # distribution = [
    # 	reverses[1] / sum_r,
    # 	distribution_1 + reverses_2 / sum_r,
    # 	…,
    # 	distribution_count-1 + reverses_count / sum_r] -
    # - Распределение для русской рулетки
